...
| `05_flow_construct_tests.R` | The nested tests of the two structural assumptions: every configural functional form against the additive challenge-plus-skill model, the composite against its constituents, the head-to-head channel contrasts with flow as the reference, and the per-person condition slopes. |

Outputs land in `src/results/tables/12_flow_*.csv` and `src/results/models/12_flow_analytic_frame.csv`.
Manuscript 02 is built from these files by pipeline stage 14.
