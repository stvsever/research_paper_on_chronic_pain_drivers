...
Runs every stage of the Viane idiographic EMA network analysis in order. Python stages run
in-process; R analysis stages are executed as standalone ``.R`` files through ``Rscript``
(see src/utils/lib/runr.py for the rationale). This is the single entry point that
reproduces all processed data, results, figures, and tables from the raw inputs.

Usage:
    python src/utils/pipeline/full/run_all.py            # run everything
    python src/utils/pipeline/full/run_all.py --from 07  # resume from a stage prefix
    python src/utils/pipeline/full/run_all.py --only 07  # run a single stage prefix
"""
...
STEPS = [
    ...
    ("02", "py", "02_descriptives/03_studyweek.py", []),
    ...
    ("08", "py", "08_baseline_trait_profiles/01_trait_profiles.py", []),
    ("09", "py", "09_network_trait_link/01_between_person.py", []),
    ("09", "R",  "09_network_trait_link/02_moderation.R", []),
    ("10", "R",  "10_robustness/01_detrended_mlvar.R", []),
    ("10", "R",  "10_robustness/02_leave_one_out.R", []),
    ("10", "R",  "10_robustness/03_person_bootstrap.R", ["200"]),
    ("10", "R",  "10_robustness/04_extended_model.R", []),
    ("10", "R",  "10_robustness/05_attention_composite.R", []),
    ...
    ("12", "R",  "12_flow_states/scripts/01_flow_construct.R", []),
...
def main():
    ...
    ap.add_argument("--from", dest="frm", default=None,
                    help="resume from this stage prefix (e.g. 07)")
